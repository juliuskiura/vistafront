...
interface InvoiceViewProps {
  ...
  /** Called when the "Confirm & Pay" button is clicked. When provided, the
   * component delegates to it instead of showing its own success modal. */
  onConfirm?: () => void;
  /** Hide the "Confirm & Pay" button (used when the caller renders its own). */
  hideConfirmButton?: boolean;
}

export default function InvoiceView({ invoice, workspaceDomain, workspaceCount, organizationName, onConfirm, hideConfirmButton }: InvoiceViewProps) {
              ...
              {!hideConfirmButton && (
                ...
                >
                  {isProcessing ? (
                    ...
                    <>
                      <span>Confirm & Pay (Ksh {(currentPrice + vat).toLocaleString()})</span>
                    </>
                  )}
                </button>
              )}
